...
function JSCompiler_StaticMethods_updateWebAnimationsInput(JSCompiler_StaticMethods_updateWebAnimationsInput$self) {
  ...
  JSCompiler_StaticMethods_updateWebAnimationsInput$self.webAnimationsInput_ = {keyframes:times.map(time => {
    ...
    var JSCompiler_object_inline_isMixed_1336 = !1;
    for (const property of JSCompiler_StaticMethods_updateWebAnimationsInput$self.JSC$2407_tracks_.keys()) {
      ...
      if (key && key.easing) {
        ...
        if (!JSCompiler_inline_result$jscomp$0) {
          ...
          JSCompiler_object_inline_isMixed_1336 = !0;
          break;
        }
      }
    }
    JSCompiler_inline_result = !JSCompiler_inline_result || JSCompiler_object_inline_isMixed_1336 ? new module$exports$ninja$anim$CssEasing$CssEasing("linear") : JSCompiler_inline_result;
    ...
    for (const property of animatableProperties) {
      JSCompiler_inline_result = goog$string$toCamelCase(property.replace("-webkit-", "")), (JSCompiler_object_inline_isMixed_1336 = JSCompiler_StaticMethods_JSC$2391_getKeyAtTime(JSCompiler_StaticMethods_updateWebAnimationsInput$self.JSC$2407_tracks_.get(property), time)) ? JSCompiler_object_inline_isMixed_1336.value && "initial" !== JSCompiler_object_inline_isMixed_1336.value && (keyframe[JSCompiler_inline_result] = JSCompiler_object_inline_isMixed_1336.value) : JSCompiler_object_inline_isMixed_1336 || 
      1 !== keyframe.offset && 0 !== keyframe.offset || (JSCompiler_object_inline_isMixed_1336 = JSCompiler_StaticMethods_updateWebAnimationsInput$self.JSC$2407_tracks_.get(property), (JSCompiler_object_inline_isMixed_1336 = 0 === keyframe.offset ? JSCompiler_StaticMethods_getFirstKey(JSCompiler_object_inline_isMixed_1336) : JSCompiler_StaticMethods_getLastKey(JSCompiler_object_inline_isMixed_1336)) && JSCompiler_object_inline_isMixed_1336.value && "initial" !== JSCompiler_object_inline_isMixed_1336.value && 
      (keyframe[JSCompiler_inline_result] = JSCompiler_object_inline_isMixed_1336.value));
    }
    return keyframe;
  ...
  for (const effect of JSCompiler_StaticMethods_shuffleTokens_$self.effects_) {
    ...
    if (!(0 >= shuffleSeed)) {
      ...
      if ("character" === tokenType) {
        tokens$jscomp$0 = JSCompiler_StaticMethods_shuffleTokens_$self.characters_.tokens;
      } else if ("word" === tokenType) {
        tokens$jscomp$0 = JSCompiler_StaticMethods_shuffleTokens_$self.words_.tokens;
      } else if ("line" === tokenType) {
        tokens$jscomp$0 = JSCompiler_StaticMethods_shuffleTokens_$self.lines_.tokens;
      } else {
        continue;
      }
...
function JSCompiler_StaticMethods_setTime_(JSCompiler_StaticMethods_setTime_$self, timeMs) {
  for (const element$jscomp$0 of JSCompiler_StaticMethods_setTime_$self.elementToAnimEvaluatorsMap_.keys()) {
    ...
    if (null != (elemValues = textEffectsEvaluator)) {
      ...
      maskValues = effect;
      var time = goog$math$clamp(timeMs, effect.startTimeMs, effect.startTimeMs + effect.durationMs), container = elemValues.container_.parentElement === elemValues.JSC$2453_element_ ? elemValues.container_ : elemValues.container_.parentElement;
      ...
      time = tokenType.JSC$2440_yBezier_.value(JSCompiler_StaticMethods_curveParameterAtValue(tokenType.JSC$2440_xBezier_, time)) * maskValues.durationMs + maskValues.startTimeMs;
      ...
      for (let i = 0; i < effect.length; i++) {
        ...
        if ("blur" === maskValues.name) {
          ...
          "scroll" === maskValues.name && (params = elemValues.JSC$2453_element_.clientHeight, delay.style.display = "inline-block", delay.style.transform = `translateY(${goog$math$lerp(params, -delay.clientHeight, ratio)}px)`, elemValues.container_.style.height = "100%", elemValues.container_.style.overflow = "hidden", elemValues.container_.style.display = "inline-block", "vanishing-point" === maskValues.parameters.scrollType && (ratio = elemValues.container_.parentElement, ratio.style.height = 
          "100%", ratio.style.overflow = "hidden", ratio.style.display = "inline-block", ratio.style.perspective = `${elemValues.JSC$2453_element_.clientHeight}px`, elemValues.container_.style.transformOrigin = "bottom", elemValues.container_.style.transform = "rotateX(30deg) scale(1, 1.2)", elemValues.container_.style["-webkit-mask-image"] = "linear-gradient(transparent 0%, black 45%)"));
        }
...
function JSCompiler_StaticMethods_setCrashCollectionUrl() {
  var JSCompiler_StaticMethods_enableCrashCollector$self = (new webmonitoring$Monitoring("Google Web Designer".replace(/ /g, "_"))).setVersion("15.2.1.0306".concat(".captureapp")).createJsErrorsReporter();
